...
void sendBinaryDataToAppScript(const char * fname) {
  ...
  uint8_t postbuffer[BUFFER_SIZE];
  int bytesRead; int totalSent = 0;

  http.setTimeout(20000); // Timeout elevado pois os buffers possuem 64kB. 
  http.begin(url);
...
void record(I2SSampler *input, const char *fname)//, int32_t *samples)
{
  int32_t* samples = (int32_t *)malloc(sizeof(int32_t) * 1024); // buffer utilizado para gravação
  input->start(); // Inicia o I2S
  File fp = SD.open(fname, "wb");

  int count_r = 0; 
  int mult = ceil(TEMPO*SAMPLE_RATE/1024)/2 ;

  int fsize = mult * 2 * 1024;
  ...
  while (turn == 1)
  {
    ...
    uint8_t acquired_data[2];

    for(int i=0; i<samples_read;i++){
      acquired_data[0] = samples[i]>>14;
      acquired_data[1] = samples[i]>>22; 
      fp.write(acquired_data,2);
    }

    count_r += samples_read;
    if (count_r >= mult*1024){turn = 0; count_r = 0;}
  ...
  fp.close();
  free(samples);
  Serial.print("Wrote: "); Serial.print(mult*1024); Serial.print(" words."); Serial.println();
  turn=1;
